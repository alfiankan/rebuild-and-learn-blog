...
khususnya `storage device`, kita tidak akan sangat dalam point yang akan kita coba recall adalah unit ukuran byte.
...
Pada contoh dibawah ini cobalah untuk melihat persamaan dari decimal dan binary dengan menggeser range dibawah.
...
Lalu kita akan recall untuk konversi ke ukuran lebih besar, karena berbasih biner maka kita bisa pangkatkan `2` dengan `10` (decimal base 10)
...
Selanjutnya coba kita perhatikan konversi koncersi dibawah, anda dapat menggunakan ini untuk sepanjang series ini karena akan sering berurusan dengan unit ini.
...
Selanjutnya kita harus juga mengetahui secara garie besar minimal bagaimana sistem operasi mengelola `I/O` terutama `Disk`, namun tanpa terlalu melebar
...
Untuk mengurangin `overhead` dalam baca tulis ke `Secondary Storage` kebanyakan sistem operasi (terutama yang jadi pebahasan dalam series ini (Linux dan Unix))
...
Buffer ini biasanya di tempatkan pada kernel memmory yang mana menggunakan 'RAM' yang jika kita lihat pada tabel sebelumnya lebih cepat
dalam kecepatan namun memang terbatas pada ukuran penyimpanan, maka dari itu sistem operasi telah memiliki
mekanisme pintar untuk mengelola ini.

Namun terkadang ini menjadi permasalahan juga pada aplikasi yang ingin menghandle process `I/O` secara lebih, karena process
os dalam melakukan `sync/flush` tidak terlihat dengan jelas, namun beberapa database yang tetap memanfaatkan buffer dari sisi os
telah memiliki mekanisme lain untuk menangani ini, untuk menggunakan buffer os kita bisa menggunakan syscall `mmap`.

:::info
`*syscall*` atau System Call adaah mekanisme memanggil layanan dari sistem operasi secara programatic. Ada banyak sekali `syscall`
pada beberapa os yang berbeda dan ada yang sama dan ada yang mengikuti standart `POSIX` (Portable Operating System Interface).
...
Beberapa database memanfaatkan `mmap` dan menggunakan syscall lain untuk mengatasi lack of control ke os buffer, salah satu artikel
yang menarik dalam pembahasan mmap ini dapat [dibaca disini](https://db.cs.cmu.edu/papers/2022/cidr2022-p13-crotty.pdf). Beberapa
database yang menggunakan `mmap` contohnya `mongodb` dan influx.
...
Karena database membutuhkan performa tinggi, maka kebanyakan database lebih menggunakan buffer nya sendiri dan memiliki
sistem management buffer sendiri yang tentu saja full control, dengan menggunakan buffe sendiri database dapat menentukan
kapan data di load ke buffer, kapan di baca tulis, kapan flush ke disk persistence dan optimasi lainya.

Pada sistem operasi (khususnya Linux dan Unix) kita dapat langsung membaca ke disk tanpa mempedulikan dan membaca dari buffer yaitu dengan
flag syscall `O_DIRECT` sehingga meminimalisir penggunaan cache kernel.
